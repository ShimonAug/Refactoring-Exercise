...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            result.append(String.format("  %s: %s (%s seats)%n",
                    getPlay(p).getName(), usd(rslt), p.getAudience()));
        }
    ...
    private int getVolumeCredits(Performance perf) {
        int rslt = 0;
        // add volume credits
        rslt += Math.max(perf.getAudience() - Constants.BASE_VOLUME_CREDIT_THRESHOLD, 0);
        // add extra credit for every five comedy attendees
        if ("comedy".equals(getPlay(perf).getType())) {
            rslt +=
                perf.getAudience() / Constants.COMEDY_EXTRA_VOLUME_FACTOR;
        }
        return rslt;
    }
